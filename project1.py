...
def get_input1()->str:
    ...
    while True:
        ...
        try:
            p1 = Path(s1[2:])
            if ((s1.startswith('D') or s1.startswith('R'))and s1[1:2]==' ' and p1.exists()and p1.is_dir()and len(s1)>2):
                break
            print('ERROR')
        except:
            print('ERROR')
...
def get_filelist2(path_list:[Path])->[Path]:
...
def get_filelist3(sec_path_list: [Path])->None:
...
def small_function(list1:[Path],i:int)->[Path]:
    'get a pathlist and int from input and return the list of path of files whose size is less than the int'
    new_list = []
    for p in list1:
...
def large_function(list1:[Path],i:int)->[Path]:
    'get a pathlist and int from input  and return the list of path of files whose size is larger than the int'
    new_list = []
    for p in list1:
        if os.path.getsize(p) > i :
            new_list.append(p)
...
def run()->None:
    ...
    sec_path_list =  get_filelist2(path_list)
    if len(sec_path_list)>0:
        get_filelist3(sec_path_list)
